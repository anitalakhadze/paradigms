...
static void displayVector(vector<string> &iter) {
  int lineLimit = 60;
  int currLineSize = 0;
...
static void expandNonTerminal(vector<string> &iter, map<string,Definition> &grammar, string nonTerminal){
  ...
  for (curr = randomProd.begin(); curr != randomProd.end(); curr++){
    string word = *curr; 
    if (word[0] == '<'){
      expandNonTerminal(iter, grammar, word);
    } else {
      iter.push_back(word); 
    }
  }
...
static void fillVector(vector<string> &iter, map<string, Definition> &grammar) {
  iter.clear();
  expandNonTerminal(iter, grammar, "<start>");
}
