...
bool previouslySeenActor(string &actor, set<string>& actors){
  return actors.find(actor) != actors.end(); 
}

bool previouslySeenFilm (film& movie, set<film> &movies) {
...
bool generateShortestPath(string source, string target, imdb &db){
  ...
  while(!partialPaths.empty() && (partialPaths.front().getLength() <= 5)){
    ...
    if (!previouslySeenActor(lastActor, previouslySeenActors)) {
      previouslySeenActors.insert(lastActor);
      vector<film> credits;
      db.getCredits(lastActor, credits);

      for (int i = 0; i < credits.size(); i++){
        film movie = credits[i]; 
        if (!previouslySeenFilm(movie, previouslySeenFilms)){
          ...
          for (int j = 0; j < players.size(); j++){
            string currPlayer = players[j]; 
            front.addConnection(movie, currPlayer); 
...
int main(int argc, const char *argv[])
{
  ...
  while (true) {
    ...
    if (source == target) {
      ...
      bool pathExists = generateShortestPath(source, target, db);
      if(!pathExists) {
