...
Report_Init(Bank *bank, AccountAmount reportAmount, int maxNumWorkers)
{
  ...
  //making nightly report system 
  bank->num_active_workers = numWorkers; 
  pthread_mutex_init(&bank->report_mutex, NULL); 
  pthread_cond_init(&bank->report_cond, NULL); 
  //bank->last_thread_finished_report = false; 
  return 0;
}
...
{
  pthread_mutex_lock(&bank->report_mutex);
  if(bank->num_active_workers > 1){
    bank->num_active_workers--; 
    printf("Thread %i is going to sleep because num_active_workers is %i\n", 
    workerNum, bank->num_active_workers);

    //while (bank->last_thread_finished_report == false){
      pthread_cond_wait(&bank->report_cond, &bank->report_mutex);
    //}
  } else {
    printf("Thread %i is doing report.\n", 
        workerNum);
    ...
    */
    ...
    //bank->last_thread_finished_report = true;
    bank->num_active_workers = numWorkers;
    pthread_cond_broadcast(&bank->report_cond);
    pthread_mutex_unlock(&bank->report_mutex);
