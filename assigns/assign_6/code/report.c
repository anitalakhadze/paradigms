...
{
  sem_wait(&(bank->lock_for_report_transfer)); 
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&(bank->lock_for_report_transfer));
...
{
  sem_wait(&(bank->for_check)); 
  bank->numWorkersHasToFinish--;
  if (bank->numWorkersHasToFinish == 0){
    sem_post(&(bank->for_check));
    Report *rpt = bank->report;
    assert(rpt); Y;
    if (rpt->numReports >= MAX_NUM_REPORTS) {
        // We've run out of report storage for the bank
        bank->numWorkersHasToFinish = numWorkers;
        for (int i = 0; i < numWorkers-1; i++){
          sem_post(&(bank->let_me_start_next_day)); 
        }
        return -1;
    }
    /*
    ...
    */
    ...
    sem_post(&(bank->for_check)); 
    sem_wait(&(bank->let_me_start_next_day)); 
  }
}
